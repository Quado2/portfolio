...
function Project(props) {
	...
	const {right, imageLink, stacks, title, description, pageLink, githubLink } = props;
	...
	return (
		...
		>
			...
			<div className=" image my-3 w-7/12 relative">
				<img className={`h-full hover:filter-none `} src={imageLink} alt={title} />
                <div className=" layer w-full h-full absolute top-0 left-0 " ></div>
			</div>
			<div className={`flex flex-col ${right? ' items-start' : ' items-end ' } justify-center 
                    top-0 w-5/12  h-full z-10 `}>
				<div className={`flex flex-col ${right? ' items-start' : ' items-end '} my-3`}>
					<h4 className="text-lg">Featured Project</h4>
					<h3 className="text-3xl text-skin-muted">{title}</h3>
				</div>
				<div className={`desc flex justify-end ${right? ' text-left' : ' text-right' } w-full bg-skin-nav p-7 my-3 rounded`}>
					<p className='text-lg w-full text-skin-muted'>{description}</p>
				</div>
				<div className={`w-4/5 my-3 justify-end ${right? ' text-left' : ' text-right' } `}>
					<h3 className='text-base text-skin-muted'>{stacks}</h3>
				</div>

				<div className={`flex `}>
					<a href={githubLink}>
						<svg className="github_svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" >
					...
					<a href={pageLink}>
						<svg className="visit_svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" >
